...
import static org.apache.jena.dboe.trans.bplustree.BPT.log;
...
public final class BPTreeNode extends BPTreePage
{
    ...
    /*package*/ BPTreePage get(int idx) {
        if ( false && BPT.logging(log) ) {
            String leafOrNode = isLeaf ? "L" : "N" ;
            BPT.log(log, "%d[%s].get(%d)", id, leafOrNode, idx) ;
        }
    ...
    public static Record insert(BPTreeNode root, Record record) {
        if ( BPT.logging(log) ) {
            BPT.log(log, "** insert(%s) / root=%d", record, root.getId()) ;
            if ( BPT.DumpTree )
                root.dump() ;
        }
  ...
  public Block getBackingBlock()           { return block ; }
    ...
    final Record internalInsert(AccessPath path, Record record) {
        ...
        if ( page.isFull() ) {
            ...
            if ( keyGT(record, records.get(idx)) ) {
                page.release() ;
...
//        promote1(y, this, idx) ;
    ...
    private static void reduceRoot(BPTreeNode root) {
        ...
        if ( root.isLeaf ) {
            if ( BPT.logging(log) )
                BPT.log(log, "reduceRoot << leaf root") ;
            return ;
        }
...
//        promote1(node, this, idx) ;
        ...
        if ( left != null && !left.isMinSize() ) {
            if ( BPT.logging(log) )
                BPT.log(log, "rebalance/shiftRight") ;
        ...
        if ( right != null && !right.isMinSize() ) {
            if ( BPT.logging(log) )
                BPT.log(log, "rebalance/shiftLeft") ;
    ...
    private void shuffleDown(int x) {
        ...
        if ( BPT.logging(log) ) {
            BPT.log(log, "ShuffleDown: i=%d count=%d MaxRec=%d", x, count, maxRecords()) ;
        ...
        if ( x == count - 1 ) {
            ...
            count-- ;
            if ( BPT.logging(log) ) {
                BPT.log(log, "shuffleDown << Clear top") ;
                BPT.log(log, "shuffleDown << %s", this) ;
            }
            internalCheckNode() ;
            return ;
