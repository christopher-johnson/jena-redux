...
import static org.apache.jena.base.atlas.lib.Alg.decodeIndex;
    ...
    static void promotePage(AccessPath path, BPTreePage page) {
        ...
        if ( BPT.CheckingNode && path != null ) {
            if ( path.getPath().size() > 2) {
                ...
                List<AccessPath.AccessStep> y = path.getPath().subList(0, path.getPath().size()-2) ;
                Optional<AccessPath.AccessStep> z = y.stream().filter(e -> e.node.isLeaf() ).findFirst() ;
                if ( z.isPresent() )
                    error("promote: Leaf %s found in path but not at the tail: %s") ;
            }
            // Check the page/index pointers
            Optional<AccessPath.AccessStep> z2 = path.getPath().stream().filter(e -> e.node.ptrs.get(e.idx) != e.page.getId()).findFirst() ;
            if ( z2.isPresent() )
